...
if [[ $# -ne 2 ]]; then
  echo "Usage: $0 <network-key> <new-admin-address>" >&2
  exit 64
fi
...
echo "Rotating admin on $NETWORK_LABEL ($NETWORK) to $NEW_ADMIN"
"$FORGE_BIN" script contracts/script/AdminActions.s.sol:AdminActions \
  --sig "rotateAdmin(address,address)" "$CONTRACT_ADDRESS" "$NEW_ADMIN" \
  --rpc-url "$RPC_URL" \
  --broadcast \
